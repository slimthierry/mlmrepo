    ...
    public function open()
    {
        ...
        while ($it->valid()) {
            ...
            if (!self::$forceIpv6 && !empty($ip4s)) {
                foreach ($ip4s as $ip) {
                    if ($this->debug) {
                        call_user_func($this->debugHandler, "Using ipv4, Connecting to $ip:$port...");
                    }
                    $r = @socket_connect($socket4, $ip, $port);
                    if ($r) {
                        if ($this->debug) {
                            call_user_func($this->debugHandler, "Using ipv4, Connected to $ip:$port!");
                        }
                        ...
                        call_user_func($this->debugHandler, "Using ipv4, Socket connect to $ip:$port failed; " . socket_strerror(socket_last_error()));
                    }
                }
            }
            $it->next();
